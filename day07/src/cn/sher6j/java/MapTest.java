...
/**
 *一、Map的实现类的结构：
 * |---Map：双列数据，存储key-value对的数据 ---类似与函数：y = f(x)
 *    |---HashMap:作为Map的主要实现类；线程不安全的，效率高；可以存储null的key和value
 *       |---LinkedHashMap：保证在遍历Map元素时，可以按照添加的顺序实现遍历
 *              原因：在原有的HashMap底层结构基础上，添加了一对指针，指向前一个和后一个元素
 *              对于频繁的遍历操作，LinkedHashMap效率高于HashMap
 *    |---TreeMap:保证按照添加的key-value对进行排序，实现排序遍历。考虑key的自然排序和定制排序
 *              底层使用红黑树
 ...
 * 面试题
 * 1.HashMap的底层实现原理？
 * 2.HashMap 和 Hashtable异同？
 * 3.CurrentHashMap 与 Hashtable的异同？
 *
 *二、Map结构的理解：
 * Map中的key：无序的、不可重复的，使用Set存储所有的key  --->key所在的类要重写equals()和hashCode() （以HashMap为例）
 * Map中的value：无序的、可重复的，使用Collection存储所有的value  --->key所在的类要重写equals()
...
//        map1.put(null, null); //NullPointerException
    ...
    public void test3() {
        ...
        System.out.println(map.size()); //输出为0，不是空指针
        System.out.println(map);
    }
